...
1、前端工程化应该看作一系列流程和规范，而不能单纯地认为是工具或者框架

2、其目标是解决前端开发和前后端协作开发过程中的难点痛点，涵盖项目的起始、开发、测试以及部署等环节。工具是前端工程化的实现媒介，而规范则是工程化的直到方针，而工作流程则是工程化的外在表现以及约束规范的载体

 

脚手架、（编码）构建、本地开发服务器、部署、工作流

 

### 一、前端工程概述

- 前端工程化经常被提及 原因：     前端范围内的客户端的功能和逻辑日渐复杂
...
- 前端工程师不仅仅是前端部分的技能要硬，也需要熟悉Web应用的整体框架，即对后端的基础知识也要具备。这有助于前后端协作和提高效率（体验在mock等）

 

前端不仅对接后端，还需要对接产品、UI。是这个非常重要的定位
...
SPA相对于传统的网站，减少了服务器渲染HTML的资源消耗

而且视图都由浏览器渲染，只传输数据，减少网络请求的资源消耗

页面路由更加快速灵活

（通过一些技术，如service worker）离线也能使用

 

不过也带来了一些问题，比如SPA需要等待JS加载完成后才能渲染后续HTML内容，等待时页面是空白的，也就是“白屏时间”。
...
这也是SPA应用需要同构开发来解决的（SSR： 服务端渲染）

 

- 大量的前端工具使用了Node.js开发的，提高了开发效率

 

3、前后端分离

- 因前端的快速发展和SPA大量应用，使得传统的前后端耦合的串行开发模式不再适用。为寻求高效协作，现在都是推行前后端分离的开发模式。
- 概念：     前端工程师和后端工程师进行明确的合理分工，改善因协作而拖慢开发进度的一种模式
...
- 测试上：     单测是相互独立的，结果不会影响对方；而集成测试则是联动的，需要测试工程师来遍历测试用例。出现问题的时候，要前端工程师来定位和处理问题（因为前端工程师是直接对接测试工程师/用户的），做好“BUG路由器”的职责（笑）
- 另外测试上，可以使用一些客户端监控系统来追踪bug，及时通知开发人员来处理
- 部署上：
...
b、测试上的快体现在使用单测来降低出现逻辑性错误，降低时间成本；准则体现在集成测试阶段对问题的快速定位

c、部署上的稳：部署是一个完整迭代周期的末尾，需要考虑缓存、测试用例没有100%覆盖的危机处理、灰度测试等。
...
编译转码、图片转base64、模块依赖分析、压缩打包、资源定位
...
场景：没分离的情况下，前端的细小bug，在更新后还需要后端工程师进行部署，效率极其低下

分离后减少耦合工作，可以提高迭代和维护效率

其高效主要体现在集成测试阶段，分离部署互不影响对方的工作
